...
class IndexMinHeap:
    ...
    def extract_min(self):
        assert self._count > 0, 'can not extract_min from empty heap'
        ret = self._data[self._indexes[1]]
        self._swap_indexes(1, self._count)
        self._reverse[self._indexes[self._count]] = 0
        self._count -= 1
        self._shift_down(1)
        return ret
    ...
    def get_min(self):
        ...
        return self._data[self._indexes[1]]

    def get_min_index(self):
        assert self._count > 0
        return self._indexes[1] - 1
    ...
    def change(self, i, new_item):
        ...
        #         self._shift_down(j)
        #         return
        self._shift_up(self._reverse[i])
        self._shift_down(self._reverse[i])

    def _shift_up(self, k):
        # 因为是最小堆，所以父亲节点的值应该要比孩子节点的值要小的
        while k > 1 and self._data[self._indexes[k // 2]] > self._data[self._indexes[k]]:
            self._swap_indexes(k, k // 2)
            k //= 2

    def _shift_down(self, k):
        while 2 * k <= self._count:
            j = 2 * k # 在此轮循环中，self._data[self._indexes[k]]和self._data[self._indexes[j]]交换位置
            if j + 1 <= self._count and self._data[self._indexes[j + 1]] < self._data[self._indexes[j]]:
                j += 1
            if self._data[self._indexes[k]] <= self._data[self._indexes[j]]:
                break
            self._swap_indexes(k, j)
            k = j
